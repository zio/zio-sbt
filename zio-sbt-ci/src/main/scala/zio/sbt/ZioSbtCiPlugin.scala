  ...
  lazy val generateReleaseDrafterWorkflowTask: Def.Initialize[Task[Unit]] =
    Def.task {
      ...
      if (enabled) {
        ...
        // this plugin (release-drafter's own README convention). Deleting it unconditionally
        // when the (default-off) feature is disabled would destroy that pre-existing, unrelated
        // file on every build using zio-sbt-ci. Only delete a copy this plugin generated itself,
        ...
        if (
          workflowFile.exists && IO
            .read(workflowFile)
            .startsWith("# This file was autogenerated using `zio-sbt-ci` plugin")
        )
          IO.delete(workflowFile)
