...
object WebsitePlugin extends sbt.AutoPlugin {
  ...
  lazy val generateReadmeTask: Def.Initialize[Task[Unit]] =
    Def.task {
      ...
      Unsafe.unsafe { implicit unsafe =>
        Runtime.default.unsafe.run(WebsitePluginUtils.makeReadme).getOrThrowFiberFailure()
      }
      val logger = streams.value.log
...
object WebsitePluginUtils {
  ...
  def makeReadme: Task[Unit] =
    for {
      template    <- readFile("README.template.md")
      mainContent <- readFile("docs/index.md").map(md => removeYamlHeader(md))
      comment =
