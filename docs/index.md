...
This is purely additive: it does not change `ci.yml`'s own triggers or jobs, and it does not publish the draft release automatically — a human still does that, which is what fires the existing `release: published`-gated release jobs. Because the feature defaults to *on*, `release-drafter.yml` is only ever written or deleted if this plugin created it in the first place — identified by the same "autogenerated" header every other generated workflow carries. A repo that already hand-maintains a `release-drafter.yml` (release-drafter's own README convention) sees that file left completely alone, whether `ciEnableReleaseDrafter` is left at its default or set explicitly either way; only a repo where the file doesn't exist yet, or where the plugin already owns it, is affected.
...
| `ciEnableReleaseDrafter` | `Boolean` | `true` | Generates `release-drafter.yml` and scaffolds `.github/release-drafter.yml` (once). Only writes/deletes a `release-drafter.yml` it generated itself, whether on or off |
